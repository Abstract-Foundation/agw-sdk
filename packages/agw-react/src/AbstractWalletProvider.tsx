...
function SmartAccountProvider(
  parameters: React.PropsWithChildren<{ appId: string }>,
) {
  const { appId, children } = parameters;
...
export function AbstractWalletProvider(
  parameters: React.PropsWithChildren<AbstractWalletProviderProps>,
) {
  const { appId, defaultChain, supportedChains, children } = parameters;
  return (
    <PrivyProvider
      appId={appId}
      config={{
